...
def add_slab(name, width, height, thickness, location=(0.0, 0.0, 0.0), rotation=None, apply_scale=False):
...
class TestArrangement(LaserityTestCase):
  ...
  def test_each_thickness_gets_its_own_sheets(self):
    ...
    self.assertFalse(sheets[keys[0]] & sheets[keys[1]], "stock thicknesses must not share a sheet")
  ...
  def test_overlapping_parts_are_flagged(self):
    thin = [
      p for p in self.settings.placements
...
class TestExport(LaserityTestCase):
  ...
  def _export(self, name="sheets.svg"):
    ...
    return sorted(
      os.path.join(self.directory, f) for f in os.listdir(self.directory) if f.endswith(".svg")
    )
...
class TestRegistration(LaserityTestCase):
  ...
  def test_every_operator_is_available(self):
    for name in (
      "scan", "clear", "set_all", "select_source", "arrange", "check",
      "export_svg", "open_editor", "editor", "placement_action",
    ):
