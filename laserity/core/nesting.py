...
def auto_nest(
    ...
    for item in prepared:
        ...
        placed = _first_fit(bins, padded_w, padded_h, allow_rotation)
        if placed is None:
            ...
            if result is None:
                unplaced.append(item["key"])
                continue
            placed = (len(bins) - 1, result)

        bin_index, (x, y, rotated) = placed
        placements.append(_compose(item, bin_index, x + margin, y + margin, rotated))

    return (placements, unplaced)


def _first_fit(bins, width, height, allow_rotation):
    """Insert into the first sheet with room for the part.

    Returns ``(sheet index, insertion result)``, or ``None`` when every sheet so
    far is full.
    """
    for index, packer in enumerate(bins):
        result = packer.insert(width, height, allow_rotation)
