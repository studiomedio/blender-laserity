...
#: How far a face normal may stray and still count as part of the flat surface.
DEFAULT_NORMAL_TOLERANCE = math.radians(2.0)
...
def analyze(bm, settings, reference_matrix=None):
    ...
    on_plane = sum(1 for d in offsets if (d - bottom) <= tolerance or (top - d) <= tolerance)
    ...
    face_area = (
        sum(face.calc_area() for face in bm.faces if abs(face.normal.dot(n_axis)) >= cos_tol) * 0.5
    )
