...
#: A part must be at least this much wider than its name, and this tall, to be
#: labelled at all.
LABEL_PADDING = 8.0
...
def placement_world_bounds(settings, placement, cached):
    ...
    identity = _View()
    points = _project(
        cached.geometry.outer_points(), placement_xform(settings, placement, identity)
    )
...
def _deferred_dress(area_ptr):
    """Re-apply :func:`_dress_area` on the next event loop pass.

    When the editor starts in an area that has just been switched to the Image
    Editor, its regions have not been rebuilt yet, so hiding the toolbar and
    selecting the sidebar tab silently do nothing. Doing it again a moment later
    catches that case; doing it twice is harmless.
    """

    def run():
        for window in bpy.context.window_manager.windows:
            for area in window.screen.areas:
                if area.as_pointer() == area_ptr and area.type == "IMAGE_EDITOR":
                    _dress_area(area)
                    area.tag_redraw()
                return None
        return None
...
def _draw_parts(op, region, settings, view, colours):
    ...
    for placement in settings.placements:
        ...
        visible.append(
            (
                ...
                part,
                cached,
                placement_xform(settings, placement, view),
                colour,
                conflicted,
            )
        )
    ...
    for placement, part, cached, xform, _colour, _conflicted in visible:
        ...
        if not 0.0 < (bx0 + bx1) * 0.5 < region.width:
            continue
        # A label wider than its own part would run across its neighbours, which
        # on a tightly packed sheet turns every name into unreadable overlap.
        width = _text_width(part.name, 10)
        if width > (bx1 - bx0) - LABEL_PADDING or (by1 - by0) < LABEL_MIN_HEIGHT:
            continue
        _text(
            (bx0 + bx1) * 0.5 - width * 0.5,
            (by0 + by1) * 0.5 - 4.0,
            part.name,
...
def _draw_hud(op, region, settings, sheets):
    ...
    _text(
        14.0,
        y,
        f"{total} part{'' if total == 1 else 's'} on {sheets} sheet{'' if sheets == 1 else 's'}",
        TEXT,
        size=12,
    )
    y -= 18.0
    ...
    if op.mode in {"MOVE", "ROTATE"}:
        hint = (
            "Move — click to confirm, Esc to cancel"
            if op.mode == "MOVE"
            else "Rotate — Ctrl snaps, Esc to cancel"
        )
...
class LASERITY_OT_editor(Operator):
    ...
    def invoke(self, context, event):
        ...
        bpy.app.timers.register(_deferred_dress(self._area_ptr), first_interval=0.05)
    ...
    def _modal_move(self, context, settings, event):
        ...
        if confirmed:
            ...
            travelled = abs(self.mouse[0] - self.press_at[0]) + abs(
                self.mouse[1] - self.press_at[1]
            )
            self.mode = "IDLE"
