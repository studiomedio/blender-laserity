...
class LaserityPart(PropertyGroup):
    ...
    thickness_key: FloatProperty(
        ...
        description=(
            "Thickness rounded to the grouping tolerance; parts sharing it are cut together"
        ),
        default=0.0,
    )
...
class LaseritySettings(PropertyGroup):
    ...
    scan_scope: EnumProperty(
        name="Scan",
        description="Which objects to look at",
        items=SCAN_SCOPE_ITEMS,
        default="VISIBLE",
    )
    ...
    method: EnumProperty(
        name="Method",
        description="How the outline is traced",
        items=METHOD_ITEMS,
        default=METHOD_AUTO,
    )
    ...
    use_min_rect: BoolProperty(
        ...
        description=(
            "Rotate each part so its tightest bounding rectangle is axis aligned before packing"
        ),
        default=True,
    )
    ...
    cut_start_layer: EnumProperty(
        ...
        description=(
            "LightBurn layer for the thinnest stock; further thicknesses take the next layers"
        ),
        items=_LAYER_ITEMS,
        default="1",
    )
    ...
    snap_size: FloatProperty(
        name="Snap Size",
        description="Grid step in millimetres",
        default=1.0,
        min=0.01,
        soft_max=50.0,
    )
    ...
    show_part_names: BoolProperty(
        name="Part Names", description="Label parts in the editor", default=True
    )
