...
class LASERITY_OT_scan(Operator):
    ...
    def execute(self, context):
        ...
        for obj in _source_objects(context, settings.scan_scope):
            ...
            part.thickness_key = detect.thickness_key(
                analysis.thickness, settings.thickness_tolerance
            )
...
class LASERITY_OT_arrange(Operator):
    ...
    def execute(self, context):
        ...
        parts = f"{placed} part{'' if placed == 1 else 's'}"
        message = f"{parts} on {sheets} sheet{'' if sheets == 1 else 's'}"
        if unplaced:
...
class LASERITY_OT_export_svg(Operator, ExportHelper):
    ...
    def execute(self, context):
        ...
        count = len(written)
        message = (
            f"Wrote {count} SVG file{'' if count == 1 else 's'} to {os.path.dirname(written[0])}"
        )
        if conflicts:
            self.report(
                {"WARNING"}, f"{message} — {conflicts} parts still overlap or hang off the sheet"
            )
        else:
            self.report({"INFO"}, message)
        return {"FINISHED"}
