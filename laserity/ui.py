...
class LASERITY_UL_parts(UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_prop, index):
        ...
        # A fixed split keeps the name readable in a narrow sidebar; letting the
        # row share space evenly truncated every name to "Wal...". No icon here
        # either — beside the include tick it just reads as a second checkbox.
        split = layout.split(factor=0.55, align=True)

        head = split.row(align=True)
        head.prop(item, "enabled", text="")
        label = head.row()
        ...
        quantity = tail.row(align=True)
        quantity.scale_x = 0.55
        quantity.prop(item, "quantity", text="")
...
class LASERITY_PT_parts(_Sidebar, Panel):
    ...
    def _draw_details(self, layout, settings, part):
        ...
        rows = [
            ("Thickness", format_thickness(part.thickness)),
            ("Stock", format_thickness(part.thickness_key)),
            ("Size", f"{part.width:.1f} × {part.height:.1f} mm"),
        ]
        if part.hole_count:
            rows.append(("Holes", str(part.hole_count)))
        rows.append(("Traced by", part.method.replace("_", " ").title()))
        cached = cache.get(part)
        if cached is not None:
            rows.append(("Material", f"{cached.geometry.net_area() / 100.0:.1f} cm²"))

        # One label/value pair per row. A two-column grid_flow fills across
        # before it fills down, which shuffled the pairs into gibberish.
        column = box.column(align=True)
...
class LASERITY_PT_editor_tools(_EditorSidebar, Panel):
    ...
    def draw(self, context):
        ...
        row = column.row(align=True)
        row.operator(
            "laserity.placement_action", text="Rotate 90°", icon="LOOP_BACK"
        ).action = "ROTATE_CCW"
        row.operator(
            "laserity.placement_action", text="", icon="LOOP_FORWARDS"
        ).action = "ROTATE_CW"
        row = column.row(align=True)
        row.operator(
            "laserity.placement_action", text="Mirror", icon="MOD_MIRROR"
        ).action = "MIRROR"
